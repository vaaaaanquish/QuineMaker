...
const STR = {
  ja: {
    ...
    sec_image: '1. 画像・動画',
    drop: '画像・動画をドラッグ＆ドロップ、またはクリックして選択',
    sec_settings: '2. 設定',
    label_lang: '言語',
    ...
    frames_est: '（{n}フレーム）',
    invert: 'コード領域を反転',
    colorize: 'Webのみ元画像の色で着色',
    ansi: 'ANSI埋込み色付きQuine出力',
    ...
    err_pick_image: '画像または動画ファイルを選んでください',
    err_load_image: '画像の読み込みに失敗しました',
    err_load_video: '動画の読み込みに失敗しました',
    status_extracting_n: 'フレームを抽出中… {i}/{n}',
    status_searching: '最適な出力幅を探索中…',
    err_no_width: '収まる出力幅が見つかりませんでした。画像や反転を変えてみてください。',
    done: '生成完了 ({w}×{rows}, {len}字)',
    done_video: '生成完了 ({n}フレーム, 絵 {w}×{g}, 全{rows}行, {len}字)。実行すると次フレームのプログラムが出力され（絵は出力の末尾）、{n}回実行すると元のプログラムに戻る周期{n}のQuineです。',
    run_hint_title: 'ターミナルでの再生方法',
    run_hint_desc: 'コピーした出力を保存し、以下を実行するとアニメーションが再生されます（Ctrl-C で停止）。1回の実行ごとに次フレームのプログラムが出力され、一周すると元に戻ります。',
    data_rows_omitted: '……（データ {x} 行を省略表示 — コピーでは全体を取得できます）……',
  ...
  en: {
    ...
    sec_image: '1. Image / Video',
    drop: 'Drag & drop an image or video, or click to choose',
    sec_settings: '2. Settings',
    label_lang: 'Language',
    label_threshold: 'Threshold',
    label_fps: 'Sampling fps',
    frames_est: '({n} frames)',
    invert: 'Invert code region',
    colorize: 'Tint with image colors (web preview only)',
    ansi: 'ANSI-colored quine output',
    ...
    err_pick_image: 'Please choose an image or video file',
    err_load_image: 'Failed to load the image',
    err_load_video: 'Failed to load the video',
    status_extracting_n: 'Extracting frames… {i}/{n}',
    ...
    done_video: 'Done ({n} frames, {w}×{g} picture, {rows} rows total, {len} chars). Running a program prints the next frame\'s program (the picture ends the output); after {n} runs the original returns — a quine of period {n}.',
    run_hint_title: 'How to play it in a terminal',
    run_hint_desc: 'Save the copied output and run the loop below to play the animation (Ctrl-C to stop). Each run prints the next frame\'s program; a full cycle returns to the original.',
    data_rows_omitted: '…… ({x} data rows collapsed — Copy still gives the full program) ……',
